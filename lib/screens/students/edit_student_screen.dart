...
class _EditStudentScreenState extends State<EditStudentScreen> {
  ...
  Future<void> _loadStudentData() async {
    ...
    if (student != null) {
      ...
      _enrollmentController.text = student.enrollmentNo ?? '';
      ...
      _medicalConditionsController.text = student.medicalConditions ?? '';
      
      _selectedSemester = student.semester?.toString();
      _selectedBloodGroup = student.bloodGroup;
      
      _existingProfileImage = student.profileImage;
      
      if (student.documents != null) {
        _existingDocuments = student.documents!.values.toList();
      }
    }
  ...
  void _removeExistingDocument(String documentUrl) {
    setState(() {
      _existingDocuments.remove(documentUrl);
    });
  }
  ...
  Future<void> _updateStudent() async {
    if (_formKey.currentState!.validate()) {
      ...
      if (_profileImage != null) {
        ...
        // String imageUrl = await uploadImage(_profileImage!);
      ...
      for (int i = 0; i < _existingDocuments.length; i++) {
        documents['doc_$i'] = _existingDocuments[i];
      }
      
      // Upload new documents
      for (int i = 0; i < _newDocuments.length; i++) {
        // String docUrl = await uploadDocument(_newDocuments[i]);
        // documents['new_doc_$i'] = docUrl;
      }
      
      updates['documents'] = documents;

      bool success = await studentProvider.updateStudent(
        widget.studentId,
        updates,
      );
      ...
      if (success && mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error: ${studentProvider.errorMessage ?? 'Failed to update student'}'),
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: CustomButton(
                      text: _currentStep == 4 ? 'Update Student' : 'Next',
                      onPressed: _currentStep == 4 ? _updateStudent : _nextStep,
                ...
                decoration: BoxDecoration(
                  ...
                  image: _profileImage != null
                      ...
                      : (_existingProfileImage != null
                          ? DecorationImage(
                              image: NetworkImage(_existingProfileImage!),
                              fit: BoxFit.cover,
                            )
                          : null),
                ),
                ...
                ..._existingDocuments.asMap().entries.map((entry) {
                  ...
                  return ListTile(
                    ...
                    trailing: IconButton(
                      ...
                      onPressed: () => _removeExistingDocument(url),
                    ),
                  );
                }).toList(),
              ],
        ...
        Container(
          ...
          child: Row(
            children: [
              ...
              Expanded(
                child: Text(
                  'You can upload additional documents. Existing documents will be kept unless deleted.',
                  style: TextStyle(
